  ...
  >
    ...
    <TabView :active-index="activeIndex">
      ...
      >
        ...
        <div class="grid">
          <div class="col-8 sm:col-12 md:col-8">
            <PageContentHeader
              title="Amortization"
              size="h6"
            />
              ...
              <template #expansion="slotProps">
                <div class="p-3">
                  ...
                  <div class="flex flex-column pl-5 gap-2">
                    ...
                    <div class="flex gap-2">
                      <label class="font-medium">Payment Reference: </label>
                      {{ slotProps.data.payment_reference ?? '---' }}
                    </div>
                    <div class="flex gap-2">
          ...
          <div class="col-4 sm:col-12 md:col-4">
            <PageContentHeader
              title="Repayments"
              size="h6"
            />
            <DataTable
              ref="table"
              :value="repaymentTransactions"
              :row-class="rowClass"
              table-style="min-width: 30rem"
              scrollable
              export-filename="Repayments"
              :loading="loadings.fetch_repayments"
              dataKey="id"
            >
              <template #header>
                <div class="flex justify-content-between flex-column sm:flex-row">
              ...
              <ColumnGroup type="footer">
                <Row>
                  ...
                  <Column
                    :footer="formatNumber(repaymentTransactions?.reduce((n, p) => n + (p?.amount ?? 0), 0) ?? 0)"
                  />
                </Row>
              </ColumnGroup>
            </DataTable>
...
const loadLoanRepayments = () => {
  loadings.value.fetch_repayments = true;

  LoanService.repaymentAccountTransactions(Number(props.loanId ?? 0))
    .then(({ data }) => {
      repaymentTransactions.value = data;
    })
    .catch((error) => {
      showApiError(error);
    })
    .finally(() => {
      loadings.value.fetch_repayments = false;
    });
};
